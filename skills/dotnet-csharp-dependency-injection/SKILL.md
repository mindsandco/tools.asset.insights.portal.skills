...
description: Use when registering or resolving services in .NET with Microsoft.Extensions.DependencyInjection — service lifetimes, keyed services, decoration, factory delegates, scope validation, hosted services, and avoiding captive dependencies.
license: MIT
metadata:
  author: https://github.com/mindsandco
  version: "1.0.0"
  domain: backend
  triggers: DI, dependency injection, AddSingleton, AddScoped, AddTransient, keyed services, IServiceCollection, IServiceProvider, BackgroundService, IHostedService, decorator pattern, captive dependency, scope validation
  role: specialist
  scope: implementation
  output-format: code
  related-skills: dotnet-core-expert, dotnet-csharp-async-patterns, dotnet-csharp-configuration, dotnet-http-client, dotnet-resilience
---
...
Cross-references: [dotnet-csharp-async-patterns] for `BackgroundService` async patterns, [dotnet-csharp-configuration] for `IOptions<T>` binding.
...
- See [dotnet-csharp-async-patterns] for async patterns in background workers
