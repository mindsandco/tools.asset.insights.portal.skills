...
description: Use when wiring up .NET configuration — Options pattern (IOptions/IOptionsMonitor/IOptionsSnapshot), validation, UserSecrets, environment-based config, custom configuration sources, and feature flags via Microsoft.FeatureManagement.
license: MIT
metadata:
  author: https://github.com/mindsandco
  version: "1.0.0"
  domain: backend
  triggers: Options pattern, IOptions, IOptionsMonitor, IOptionsSnapshot, IConfiguration, UserSecrets, FeatureManagement, appsettings, environment variables, ValidateDataAnnotations, ValidateOnStart, configuration binding
  role: specialist
  scope: implementation
  output-format: code
  related-skills: dotnet-core-expert, dotnet-csharp-dependency-injection
---
...
Cross-references: [dotnet-csharp-dependency-injection] for service registration patterns.
...
User secrets are stored in `~/.microsoft/usersecrets/<UserSecretsId>/secrets.json`.
