...
description: Use when adding fault tolerance to .NET applications — Polly v8 + Microsoft.Extensions.Http.Resilience, the standard resilience pipeline (rate limiter, total timeout, retry, circuit breaker, attempt timeout), and custom pipeline configuration for non-HTTP workloads.
license: MIT
metadata:
  author: https://github.com/mindsandco
  version: "1.0.0"
  domain: backend
  triggers: Polly, resilience, retry, circuit breaker, timeout, rate limiter, fault tolerance, ResiliencePipeline, AddStandardResilienceHandler, Microsoft.Extensions.Http.Resilience, transient failures
  role: specialist
  scope: implementation
  output-format: code
  related-skills: dotnet-http-client, dotnet-csharp-dependency-injection, dotnet-csharp-async-patterns, dotnet-core-expert
---
...
**Out of scope:** DI container mechanics and service lifetimes -- see [dotnet-csharp-dependency-injection]. Async/await patterns and cancellation token propagation -- see [dotnet-csharp-async-patterns]. HTTP client factory patterns (typed clients, named clients, DelegatingHandlers) are covered in [dotnet-http-client]. Testing resilience policies -- see [test-driven-development] for testing with WebApplicationFactory.

Cross-references: [dotnet-csharp-dependency-injection] for service registration, [dotnet-csharp-async-patterns] for cancellation token propagation, [dotnet-http-client] for applying resilience to HTTP clients.
...
5. **Do not use hardcoded retry delays in production** -- always use `UseJitter = true` and configure delays via IOptions.
