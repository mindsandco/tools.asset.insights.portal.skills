...
description: Use when calling external HTTP APIs from .NET — IHttpClientFactory, typed and named clients, DelegatingHandler chains for cross-cutting concerns, resilience pipeline integration, and avoiding socket exhaustion or DNS staleness.
license: MIT
metadata:
  author: https://github.com/mindsandco
  version: "1.0.0"
  domain: backend
  triggers: HttpClient, IHttpClientFactory, typed client, named client, DelegatingHandler, HttpMessageHandler, REST client, HTTP API, socket exhaustion, DNS staleness, AddHttpClient
  role: specialist
  scope: implementation
  output-format: code
  related-skills: dotnet-resilience, dotnet-csharp-dependency-injection, dotnet-csharp-async-patterns, dotnet-core-expert, testcontainers-dotnet
---
...
**Out of scope:** DI container mechanics and service lifetimes -- see [dotnet-csharp-dependency-injection]. Async/await patterns and cancellation token propagation -- see [dotnet-csharp-async-patterns]. Resilience pipeline configuration (Polly v8, retry, circuit breaker, timeout strategies) is owned by [dotnet-resilience]. Integration testing frameworks -- see [test-driven-development] for WebApplicationFactory and HTTP client testing patterns.

Cross-references: [dotnet-resilience] for resilience pipeline configuration, [dotnet-csharp-dependency-injection] for service registration, [dotnet-csharp-async-patterns] for async HTTP patterns.
...
Apply resilience to HTTP clients using `Microsoft.Extensions.Http.Resilience`. See [dotnet-resilience] for detailed pipeline configuration, strategy options, and migration guidance.
...
See [dotnet-resilience] for when to use hedging vs standard retry.
...
```csharp
// See [test-driven-development] for WebApplicationFactory patterns
```
...
- **Apply resilience via pipeline** -- use `AddStandardResilienceHandler()` (see [dotnet-resilience]) rather than manual retry loops
