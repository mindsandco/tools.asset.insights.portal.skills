...
metadata:
  ...
  version: "1.1.0"
  domain: devops
  triggers: DevOps, CI/CD, deployment, Docker, Kubernetes, Terraform, GitHub Actions, NUKE, NUKE Build, build automation, infrastructure, platform engineering, incident response, on-call, self-service
  role: engineer
  scope: implementation
  output-format: code
  related-skills: shipping-and-launch, dotnet-core-expert, dotnet-code-analyzer, jetbrains-lint-pro, testcontainers-dotnet
---
...
- Build automation for .NET projects (NUKE Build — typed C# build scripts that run identically locally and in CI)
...
| NUKE Build | `references/nuke-build.md` | Authoring or maintaining a typed C# build (`build/_build.csproj`, `.nuke/`, `build.{cmd,sh,ps1}`) for a .NET solution |
...
For full target catalogue (Pack, Push, Coverage, PublishContainer, GitVersion integration, GitHub Actions auto-generation), see `references/nuke-build.md`.
...
GitHub Actions, GitLab CI, Jenkins, CircleCI, NUKE Build, Cake, FAKE, Docker, Kubernetes, Helm, ArgoCD, Flux, Terraform, Pulumi, Crossplane, AWS/GCP/Azure, Prometheus, Grafana, PagerDuty, Backstage, LaunchDarkly, Flagger
