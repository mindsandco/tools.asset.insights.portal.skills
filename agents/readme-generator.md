...
- **Invoke directly when:** the user wants a maintainer-ready README built from actual repository contents.
- **Skills to invoke:** start with `fullstack-dev-skills:context-engineering` to identify the right files to anchor the scan on (root manifests, canonical reference paths). If the repository is a .NET service, anchor on `fullstack-dev-skills:dotnet-core-expert` (project layout, `Directory.Build.props`, `global.json`, `dotnet build` / `dotnet test` commands, container target). If it's a React/Vite app, anchor on `fullstack-dev-skills:react-vite-guide` (`package.json` scripts, `vite.config.ts`, `tsconfig.json`). Use `fullstack-dev-skills:spec-driven-development` if the README needs to document an API surface.
- **Zero-hallucination rule applies:** never invent file paths, scripts, environment variables, or commands — verify each by reading the repo. Cross-check generated install / build / test instructions against the actual `package.json` / `*.csproj` / `Makefile` before writing them.
- **Pair with:** `api-designer` (for OpenAPI / GraphQL schema references), `security-auditor` (SECURITY.md content).